...
#
# Usage: ./create-release-notes.sh <semver> <package_id> [github_nuget_source] [myget_source]
# Example: ./create-release-notes.sh "7.0.0-alpha.1" "Our.Umbraco.Slimsy" "https://nuget.pkg.github.com/Jeavon/index.json" "https://www.myget.org/F/umbraco-packages/api/v3/index.json"
...
if [ -z "$SEMVER" ] || [ -z "$PACKAGE_ID" ]; then
  ...
  echo "Usage: $0 <semver> <package_id> [github_nuget_source] [myget_source]"
  exit 1
fi

echo "Processing GitHub release for v$SEMVER (package: $PACKAGE_ID)"

# Determine installation commands based on release type
IS_PRERELEASE=false
if [[ "$SEMVER" == *"-"* ]]; then
  IS_PRERELEASE=true
  
  # Pre-release: provide GitHub Packages and optionally MyGet
  if [ -n "$GITHUB_NUGET_SOURCE" ]; then
    INSTALL_CMD_GITHUB="dotnet add package $PACKAGE_ID --version $SEMVER --source $GITHUB_NUGET_SOURCE"
  else
    INSTALL_CMD_GITHUB="dotnet add package $PACKAGE_ID --version $SEMVER"
  fi
  
  # Only prepare MyGet command if source is provided
  if [ -n "$MYGET_SOURCE" ]; then
    INSTALL_CMD_MYGET="dotnet add package $PACKAGE_ID --version $SEMVER --source $MYGET_SOURCE"
  fi
else
...
if gh release view "v$SEMVER" >/dev/null 2>&1; then
  ...
  if [[ ! "$RELEASE_NOTES" == *"dotnet add package"* ]]; then
    ...
    if [ "$IS_PRERELEASE" = true ]; then
      echo "### From GitHub Packages" >> notes.md
      echo '```bash' >> notes.md
      echo "$INSTALL_CMD_GITHUB" >> notes.md
      echo '```' >> notes.md
      if [ -n "$MYGET_SOURCE" ]; then
        echo "" >> notes.md
        echo "### From MyGet" >> notes.md
        echo '```bash' >> notes.md
        echo "$INSTALL_CMD_MYGET" >> notes.md
        echo '```' >> notes.md
      fi
    else
      echo '```bash' >> notes.md
